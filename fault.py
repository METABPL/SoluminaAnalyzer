...
def generate_fault_list(fault_list):
    ...
    print("<faultList formatVersion=\"1.3\" timestamp=\"{}\">".format(timestamp), file=outfile)
    ...
    for fault in fault_list:
        ...
        if fault.tool is not None:
            print("      <src>{}</src>".format(fault.tool), file=string_out)
        elif fault.item is not None:
            print("      <src>{}</src>".format(fault.item), file=string_out)
        print("    </faultOriginLocation>", file=string_out)
        if len(fault.path) > 0:
        ...
        print("  <fault id=\"TA1_metaBPL_UC1_{}_Fault_{}\">".format(fault.process.tdp, fault_hash), file=outfile)
